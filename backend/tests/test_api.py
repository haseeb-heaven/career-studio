...
def test_patch_profile(client):
    pid = _import_json(client)
    resp = client.patch(f"/api/profiles/{pid}", json={"full_name": "Jane Updated"})
    assert resp.status_code == 200
...
def test_export_formats(client, fmt, mime):
    pid = _import_json(client)
    resp = client.get(f"/api/profiles/{pid}/export/{fmt}")
    assert resp.status_code == 200
...
def test_export_invalid_format(client):
    pid = _import_json(client)
    resp = client.get(f"/api/profiles/{pid}/export/xyz")
    assert resp.status_code == 400
